/* Agon-ized version by Shawn Sijnstra Copyright (c) 2024
 * amines.c - core of the game.
 * GPLv3 license
 *
 * based on cmines.c -
...
static char const      *yowzitch =
	...
	"   -s  Read setups from SETUPFILE (default is ./.aminesrc)\n"
...
static char const      *vourzhon =
    "amines, version 0.1. Copyright (C) 2024 by Shawn Sijnstra\n"
    "ported from:\n"
 ...
 */
static char const	defaultsetup[] = "Beginner: 8 8 10 0\r\n"
...
//	    die("%s: %s", setupfile, strerror(errno));
	...
	// next line is in place because EOF doesn't seem to work on real hardware
	if ((setups[setupcount].name[0] == '\n') || (setups[setupcount].name[0] == '\r') || (setups[setupcount].name[0] == '\0')) break;
	...
	printf("Reading setup %s count %d\n\r",setups[setupcount].name,setupcount);
	++setupcount;
    }
    fclose(fp);
...
static void initwithcmdline(int argc, char *argv[], startupdata *start)
{
    ...
    if (!setupfile)
//	setupfile = makepathname(getenv("HOME"), \".aminesrc");
    setupfile = ".aminesrc";
}
